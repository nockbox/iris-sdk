 ...
 */
export function evmAddressToBelts(address) {
...
export function bigintToAtom(n) {
    if (n === 0n)
        return '0';
 ...
 */
export function buildBridgeNoun(evmAddress, config) {
 ...
 */
export function verifyBeltEncoding(address) {
    if (!isEvmAddress(address))
        return false;
 ...
 */
export function isBridgeConfigured(config) {
 ...
 */
export async function createBridgeNoteData(evmAddress, config) {
 ...
 */
export async function buildBridgeTransaction(params, config) {
    ...
    const noteData = [[config.noteDataKey, bridgeNounJs]];
    const bridgeSpendCondition = [
        { Pkh: { m: config.threshold, hashes: config.addresses } },
    ];
    const bridgeLockRoot = { Lock: bridgeSpendCondition };
    const refundLock = [{ Pkh: { m: 1, hashes: [params.refundPkh] } }];
 ...
 */
export async function validateBridgeTransaction(rawTxProto, config) {
    try {
        const rawTx = wasm.rawTxFromProtobuf(rawTxProto);
        ...
        const outputData = outputs.map((output) => {
            const noteData = 'note_data' in output ? (output.note_data ?? []) : [];
            return {
        ...
        for (const output of outputData) {
            const hasKey = output.noteData?.some((entry) => entry[0] === config.noteDataKey);
            if (hasKey) {
                bridgeOutput = output;
                break;
            }
        ...
        const bridgeEntryPair = bridgeOutput.noteData.find((entry) => entry[0] === config.noteDataKey);
        if (!bridgeEntryPair) {
            return {
                valid: false,
        ...
        try {
            const value = bridgeEntryValue;
            const decoded = Array.isArray(value)
                ? value
                : wasm.cue(value instanceof Uint8Array ? value : new Uint8Array(value));
            ...
            return {
                valid: false,
                error: `Failed to decode bridge note data: ${err instanceof Error ? err.message : String(err)}`,
            };
        }
        return {
            valid: true,
 ...
 */
export async function assertValidBridgeTransaction(rawTxProto, context, config) {
